...
- **Markdown 文件**：要转换的 `.md` 文件路径。支持多个文件、通配符（`*.md`）
- **排除文件**（可选）：当使用通配符时，可通过 `--exclude` 排除特定文件
- **输出文件/目录**（可选）：单文件默认与输入同名的 `.docx`；多文件可指定输出目录
...
page:
  ...
  page_number:
    enabled: true                    # 是否添加页码
    start_at: 1                      # 从文档第几页开始编页码（1=从首页开始，3=第3页开始编"1"）
```
...
多文件 / 通配符转换（同一个配置应用于所有文件）：

```bash
md2docx *.md -c config.yaml -o <输出目录/>
md2docx a.md b.md c.md -c config.yaml
```

排除特定文件：

```bash
md2docx *.md -c config.yaml --exclude README.md --exclude "草稿*"
```
...
| 目录到三级标题 | `toc.level: 3` |
| A4纸 | `page.size: "A4"` |
| 不要页码 | `page.page_number.enabled: false` |
| 加页码 | `page.page_number.enabled: true` |
| 从第3页开始编页码 | `page.page_number.start_at: 3` |
...
| 用户说 | 意味着 |
|--------|--------|
| 「学术风格」「学术论文」 | 正文宋体12pt, 单倍行距, 首行缩进2, 两端对齐; 标题黑体加粗（h1三号居中，h2四号居中，h3+小四居左）; 三线表; 带页码 |
| 「课程报告」「实验报告」 | 含封面, 目录到三级, 学术风格正文; 带页码（如有封面+目录则从第3页开始编"1"） |
| 「读书笔记」 | 无封面, 无目录, 正文楷体12pt, 1.5行距; 带页码 |
...
**用户**：`/md2docx 把 test 目录下所有 .md 都转成 Word，除了实验一.md，学术风格`

**你的处理**：

1. 输入 = `test/*.md`，排除 = `实验一.md`
2. 用户说「学术风格」→ 使用默认模板值即可（正文宋体12pt, 单倍行距, 首行缩进2, 两端对齐; 标题黑体加粗）
3. 写入 `config.yaml`（使用默认值）
4. 执行 `md2docx test/*.md -c config.yaml --exclude "实验一.md"`
5. 提醒更新目录域
...
- 多文件转换时，同一个 `config.yaml` 会应用到所有文件。如果需要不同的配置，请分次执行
- 当用户说「批量转换」「所有 md」「这几个文件」「排除某个」时 → 使用通配符或多文件 + `--exclude`
